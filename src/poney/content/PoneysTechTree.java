...
import static poney.content.PoneysBlocks.*;

public class PoneysTechTree {
    private static TechNode context = null;

    public static void load() {

        mergeNode(plastaniumConveyor, () -> {
            node(poneysConveyor);
        });

        mergeNode(pulseConduit, () -> {
            node(pressurizedConduit, () -> {
                node(pressurizedLiquidRouter);
                node(pressurizedLiquidJunction);
                node(pressurizedBridgeConduit);
                node(pressurizedPhaseConduit);
            });
        });
    }
